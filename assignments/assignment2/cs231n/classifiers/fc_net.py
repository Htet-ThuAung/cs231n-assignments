...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i, hidden_dim in enumerate(hidden_dims):
            ...
            if self.normalization in ['batchnorm', 'layernorm']:
                ...
                self.params[f'beta{i+1}'] = np.ones(hidden_dim)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            # Apply batchnorm/layernorm if enabled
            if self.normalization == "batchnorm":
        ...
        # Last affine layer (output layer)
        ...
        #print(f"y shape: {y.shape}, unique y: {np.unique(y)}")
        #print(f"Label values: {np.min(y)}, {np.max(y)}")
        ...
        # Backprob through hidden layers
        for i in range(self.num_layers - 1, 0, -1):
            # Apply dropout backward if enabled
            if self.use_dropout:
                dout = dropout_backward(dout, dropout_caches.pop())

            # Backprob through relu
            dout = relu_backward(dout, caches.pop())

            # Backprop through batch/layer normalization if enabled
            if self.normalization == "batchnorm":
