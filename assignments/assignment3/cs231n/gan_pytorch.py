...
def build_dc_classifier(batch_size):
    ...
    Architecture:
        - Conv2D: 32 filters, 5x5 kernel, stride 1
        - LeakyReLU(0.01)
        - MaxPool2D: 2x2, stride 2
        - Conv2D: 64 filters, 5x5 kernel, stride 1
        - LeakyReLU(0.01)
        - MaxPool2D: 2x2, stride 2
        - Flatten
        - FC: 4*4*64
...
def build_dc_generator(noise_dim=96):
    return nn.Sequential(
        # Fully connected to 1024
        nn.Linear(noise_dim, 1024),
        nn.ReLU(),
        nn.BatchNorm1d(1024),

        # Fully connected to 7*7*128
        nn.Linear(1024, 7 * 7 * 128),
        nn.ReLU(),
        nn.BatchNorm1d(7 * 7 * 128),

        # Unflatten to (batch_size, 128, 7, 7)
        nn.Unflatten(1, (128, 7, 7)),

        # ConvTranspose2d: 128 → 64, output shape becomes (batch_size, 64, 14, 14)
        nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1),
        nn.ReLU(),
        nn.BatchNorm2d(64),

        # ConvTranspose2d: 64 → 1, output shape becomes (batch_size, 1, 28, 28)
