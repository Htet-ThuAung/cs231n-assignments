...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        caption_embeddings = self.embedding(captions) # (N, T, d_model)
        caption_embeddings = self.positional_encoding(caption_embeddings)
        #caption_embeddings += self.positional_encoding(caption_embeddings).to(device)

        # Project image features to d_model dimension
        image_embeddings = self.visual_projection(features).unsqueeze(1) # (N, 1, d_model)

        # Prepare a casual mask to prevent attending to future positions
        tgt_mask = torch.tril(torch.ones(T, T, device=device)).bool()  # Shape should be (T, T)
        # tgt_mask = tgt_mask.unsqueeze(0)  # This gives a shape of (1, T, T)
        # tgt_mask = tgt_mask.expand(N, T, T)  # Expands to (N, T, T)
