...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # Word embedding lookup for captions_in
        ...
        if self.cell_type == "rnn":
            ...
            h, cache_rnn = lstm_forward(x, h0, Wx, Wh, b) # Shape: (N, T, H)

        # Compute vocabulary scores
        scores, cache_affine = temporal_affine_forward(h, W_vocab, b_vocab)  # Shape: (N, T, V)
    ...
    def sample(self, features, max_length=30):
        ...
        h = features @ W_proj + b_proj # Initial hidden state (N, H)
        c = np.zeros_like(h) if self.cell_type == "lstm" else None # Initial cell state
        word = np.full((N,), self._start, dtype=np.int32) # Start token
        ...
        for t in range(max_length):
            ...
            word = np.argmax(scores, axis=1) # Selectt highest scoring word
            captions[:, t] = word # Store sampled word
