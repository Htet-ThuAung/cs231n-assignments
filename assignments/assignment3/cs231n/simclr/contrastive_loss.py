...
def sim(z_i, z_j):
    ...
    # Compute the dot product between z_i and z_j
    dot_product = torch.dot(z_i.squeeze(), z_j.squeeze())

    # Compute the norms of z_i and z_j
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        # Compute loss for (k, k+N) and (k+N, k)
        loss_k = -torch.log(numerator / denominator_k)

        numerator_N = torch.exp(sim(z_k_N, z_k) / tau)
        
        denominator_k_N = 0
        for j in range(2 * N):
            if j != k + N:
                denominator_k_N += torch.exp(sim(z_k_N, out[j]) / tau)
            loss_k_N = -torch.log(numerator_N / denominator_k_N)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # Normalize vectors
    out_left_norm = out_left / norm_left
    out_right_norm = out_right / norm_right

    # Compute dot product between corresponding pairs
...
def compute_sim_matrix(out):
    ...
    # Compute L2 norm for each row vector
    norms = torch.linalg.norm(out, dim=1, keepdim=True)

    # Normalize each row vector
    out_norm = out / norms

    # Compute similarity matrix as dot product between normalized vectors
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    mask = (torch.ones_like(exponential) - torch.eye(2 * N, device=out.device)).bool()
